...
plA<-ggplot(df,aes(as.numeric(x),as.numeric(y),color=model))+geom_line()+scale_color_manual(values=pal)+scale_x_continuous(trans="log10")+xlab(expression("selection coefficient (|" * italic(s) * "|)"))+ylab(expression(beta))+theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
